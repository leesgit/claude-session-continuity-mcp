...
interface StatsResult {
  ...
  memoryTypes: Array<{ memory_type: string; count: number }>;
...
function getStats(): StatsResult {
  const memoriesCount = (db.prepare('SELECT COUNT(*) as count FROM memories').get() as { count: number }).count;
  const sessionsCount = (db.prepare('SELECT COUNT(*) as count FROM sessions').get() as { count: number }).count;
  const relationsCount = (db.prepare('SELECT COUNT(*) as count FROM memory_relations').get() as { count: number }).count;
  const patternsCount = (db.prepare('SELECT COUNT(*) as count FROM work_patterns').get() as { count: number }).count;

  let embeddingsCount = 0;
  try {
    embeddingsCount = (db.prepare('SELECT COUNT(*) as count FROM embeddings').get() as { count: number }).count;
  } catch { /* ignore */ }

  const memoryTypes = db.prepare('SELECT memory_type, COUNT(*) as count FROM memories GROUP BY memory_type').all() as Array<{ memory_type: string; count: number }>;
  const projects = db.prepare('SELECT DISTINCT project FROM memories WHERE project IS NOT NULL UNION SELECT DISTINCT project FROM sessions WHERE project IS NOT NULL').all() as Array<{ project: string }>;
  ...
  const recentActivity = db.prepare(`
    ...
    LIMIT 5
  `).all() as Array<{ type: string; content: string; timestamp: string; project: string }>;
...
function getProjectContext(project: string): ProjectContext | null {
  try {
    const projectContext = db.prepare('SELECT * FROM project_context WHERE project = ?').get(project) as {
    ...
    const activeContext = db.prepare('SELECT * FROM active_context WHERE project = ?').get(project) as {
...
function getMemories(params: URLSearchParams) {
...
function getMemory(id: number) {
  return db.prepare('SELECT * FROM memories WHERE id = ?').get(id);
}

function updateMemory(id: number, data: { content?: string; tags?: string[]; importance?: number; memory_type?: string }) {
  ...
  if (updates.length === 0) return { success: false, message: 'No updates' };
...
function deleteMemoryById(id: number) {
  const result = db.prepare('DELETE FROM memories WHERE id = ?').run(id);
  return { success: result.changes > 0 };
}

function getTimeline(project?: string) {
  let sql = `
    SELECT
...
function getRelations(memoryId?: number) {
  if (memoryId) {
    return db.prepare(`
      SELECT r.*,
  ...
  try {
    ...
    if (pathname.startsWith('/api')) {
      ...
      if (memoryMatch) {
        ...
        if (req.method === 'GET') return json(getMemory(id));
        if (req.method === 'PUT') {
          const body = await parseBody();
          return json(updateMemory(id, body as Parameters<typeof updateMemory>[1]));
        }
        if (req.method === 'DELETE') return json(deleteMemoryById(id));
      }
