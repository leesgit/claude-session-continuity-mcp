...
export async function handleProjects(args: unknown): Promise<CallToolResult> {
...
async function getProjectList(): Promise<CallToolResult> {
  // 파일 시스템에서 프로젝트 목록 조회
  ...
  const totalTasks = (db.prepare('SELECT COUNT(*) as count FROM tasks WHERE status != \'done\'').get() as { count: number }).count;
...
async function getProjectDetail(project: string): Promise<CallToolResult> {
  ...
  `).all(project) as Array<{ id: number; content: string; memory_type: string; created_at: string }>;
...
function detectPlatform(projectPath: string): string {
  if (fs.existsSync(path.join(projectPath, 'pubspec.yaml'))) return 'flutter';
  if (fs.existsSync(path.join(projectPath, 'build.gradle')) || fs.existsSync(path.join(projectPath, 'build.gradle.kts'))) return 'android';
