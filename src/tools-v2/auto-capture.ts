...
import { invalidateContext, invalidateProjects } from '../utils/cache.js';
...
export const autoCaptureTools: Tool[] = [
  {
    name: 'session_start',
...
export async function handleSessionStart(args: unknown): Promise<CallToolResult> {
  return logger.withTool('session_start', async () => {
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `Session start failed: ${error}` }],
        isError: true
      };
    }
...
export async function handleSessionEnd(args: unknown): Promise<CallToolResult> {
  return logger.withTool('session_end', async () => {
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `Session end failed: ${error}` }],
        isError: true
      };
    }
  }, args as Record<string, unknown>);
...
export async function handleSessionSummary(args: unknown): Promise<CallToolResult> {
  return logger.withTool('session_summary', async () => {
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `Session summary failed: ${error}` }],
        isError: true
      };
    }
  }, args as Record<string, unknown>);
...
function recordSessionStart(project: string): void {
  try {
...
function recordSessionEnd(project: string, lastWork: string, verification?: string): void {
  try {
