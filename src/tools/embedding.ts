...
export async function rebuildEmbeddings(force: boolean = false): Promise<CallToolResult> {
  try {
    ...
    const stmt = db.prepare(sql);
    const rows = stmt.all() as Array<{ id: number; content: string }>;
...
export function getEmbeddingStatus(): CallToolResult {
  try {
