...
export function recordFilterPattern(
  patternType: string,
  ...
  try {
    ...
    `);
    const existing = existingStmt.get(patternType, patternDescription) as { id: number; occurrence_count: number } | undefined;
...
export function getSafeOutputGuidelines(context?: string): CallToolResult {
  try {
