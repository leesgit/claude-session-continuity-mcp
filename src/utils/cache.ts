...
// 컨텍스트 캐시 (자주 조회)
export const contextCache = new QueryCache<unknown>({
  maxSize: 50,
  ttlMs: 60000 // 1분
});

// 메모리 검색 캐시
export const memoryCache = new QueryCache<unknown>({
  maxSize: 100,
  ttlMs: 30000 // 30초
});
...
export function makeContextKey(project: string): string {
  return `context:${project}`;
}

export function makeMemoryKey(query: string, project?: string, type?: string): string {
  return `memory:${project || '*'}:${type || '*'}:${query}`;
}

export function makeProjectKey(): string {
  return 'projects:list';
}

// ===== 캐시 무효화 트리거 =====

export function invalidateContext(project: string): void {
  contextCache.invalidate(makeContextKey(project));
}

export function invalidateMemory(project?: string): void {
  if (project) {
