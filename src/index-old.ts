...
let contentFilterPatterns: Array<{
  id: number;
  patternType: string;
...
function parseMarkdownTable(content: string, tableName: string): Record<string, string> {
...
async function getTechStack(project: string): Promise<CallToolResult> {
...
async function runVerification(project: string, gates?: string[]): Promise<CallToolResult> {
...
async function detectPlatform(project: string): Promise<CallToolResult> {
  const projectPath = path.join(APPS_DIR, project);
...
function searchSimilarWork(keyword: string, project?: string): CallToolResult {
  try {
    let query = `
...
function getProjectStats(project?: string): CallToolResult {
  try {
    let query: string;
    let params: string[] = [];
...
function getWorkPatterns(project?: string, workType?: string): CallToolResult {
  try {
...
function recallMemory(
  ...
  try {
    ...
    `;
    const params: (string | number)[] = [ftsQuery, minImportance];
    ...
    const rows = stmt.all(...params) as (MemoryRow & { rank: number })[];
    ...
    try {
      ...
      const params: (string | number)[] = [likePattern, likePattern, minImportance];
...
function searchByTag(
  ...
  try {
    ...
    const allRows = stmt.all() as MemoryRow[];
...
function findConnectedMemories(
  memoryId: number,
  depth: number = 1,
  relationType?: string
): CallToolResult {
  try {
    ...
    const findAtDepth = (currentIds: number[], currentDepth: number) => {
...
function getMemoryStats(): CallToolResult {
  try {
    ...
    const byTypeStmt = db.prepare(`
      ...
      ORDER BY count DESC
    `);
    const byType = byTypeStmt.all() as Array<{ memory_type: string; count: number }>;
    ...
    const byProjectStmt = db.prepare(`
      ...
      ORDER BY count DESC
    `);
    const byProject = byProjectStmt.all() as Array<{ project: string; count: number }>;

    const relationsStmt = db.prepare('SELECT COUNT(*) as count FROM memory_relations');
    const relationsCount = (relationsStmt.get() as { count: number }).count;
    ...
    const recentStmt = db.prepare(`
      ...
      LIMIT 5
    `);
    const recentRows = recentStmt.all() as MemoryRow[];
    ...
    const mostAccessedStmt = db.prepare(`
      ...
      LIMIT 5
    `);
    const mostAccessedRows = mostAccessedStmt.all() as MemoryRow[];
...
function deleteMemory(memoryId: number): CallToolResult {
  try {
    // 관계도 함께 삭제 (CASCADE)
...
async function semanticSearch(
  ...
  try {
    ...
    const stmt = db.prepare(sql);
    const rows = stmt.all(...params) as (MemoryRow & { memory_id: number; embedding: Buffer })[];
...
async function rebuildEmbeddings(force: boolean = false): Promise<CallToolResult> {
  try {
...
function getEmbeddingStatus(): CallToolResult {
  try {
...
const FEEDBACK_IMPORTANCE: Record<string, number> = {
  'bug': 8,
  'timeout': 7,
  'performance': 6,
  'feature-request': 5,
...
async function collectWorkFeedback(
  project: string,
  workSummary: string,
  feedbackType: string,
  ...
  try {
    ...
    if (feedbackType !== 'none' && feedbackContent) {
      ...
      const memoryStmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `);
      const memoryResult = memoryStmt.run(
        content,
        'feedback',
        JSON.stringify(tags),
        'project-manager-mcp',
      ...
      result.feedbackId = memoryResult.lastInsertRowid as number;
  ...
  try {
    ...
    const feedbacks = rows.map(row => {
      ...
      return {
        ...
        type: tags.find((t: string) => ['bug', 'timeout', 'feature-request', 'ux', 'performance'].includes(t)) || 'unknown',
...
function resolveFeedback(
  ...
  try {
    ...
    const existing = checkStmt.get(feedbackId, 'feedback') as MemoryRow | undefined;
...
function recordFilterPattern(
  ...
  try {
    ...
    `);
    const existing = existingStmt.get(patternType, patternDescription) as {
      id: number;
...
function getProjectContext(project: string): CallToolResult {
  try {
    // Layer 1: 고정 컨텍스트
    const fixedStmt = db.prepare(`SELECT * FROM project_context WHERE project = ?`);
    const fixed = fixedStmt.get(project) as {
    ...
    const active = activeStmt.get(project) as {
    ...
    const tasksStmt = db.prepare(`
      ...
      LIMIT 3
    `);
    const topTasks = tasksStmt.all(project) as Array<{ id: number; title: string; status: string }>;
    ...
    const countStmt = db.prepare(`
      ...
      WHERE project = ? AND status IN ('pending', 'in_progress', 'blocked')
    `);
    const countResult = countStmt.get(project) as { count: number };
...
function updateActiveContext(
  ...
  try {
    ...
    `);
    const activeTasks = tasksStmt.all(project) as Array<{ id: number; title: string; status: string }>;
...
function updateArchitectureDecision(project: string, decision: string): CallToolResult {
  try {
    // 기존 결정들 조회
    const selectStmt = db.prepare(`SELECT architecture_decisions FROM project_context WHERE project = ?`);
    const row = selectStmt.get(project) as { architecture_decisions: string | null } | undefined;
...
function completeTask(taskId: number): CallToolResult {
  try {
    ...
    const task = taskStmt.get(taskId) as { project: string; title: string } | undefined;
...
function getPendingTasks(project: string, includeBlocked: boolean = true): CallToolResult {
  try {
    ...
    const tasks = stmt.all(project) as Array<{
      id: number;
      title: string;
...
function syncActiveTasksToContext(project: string): void {
  try {
    const tasksStmt = db.prepare(`
      SELECT id, title, status FROM tasks
      WHERE project = ? AND status IN ('in_progress', 'blocked')
...
function findSolution(errorText: string, project?: string): CallToolResult {
  try {
    // 1. 정확한 시그니처 매칭
    let stmt = db.prepare(`
    ...
    const solutions = (results as Array<{
      id: number;
      project: string | null;
...
// 컨텍스트 접근 기록용 (평가에 사용)
function recordContextAccess(project: string, accessType: 'get' | 'update'): void {
  try {
    ...
    `);
    stmt.run(
      `Context ${accessType}: ${project}`,
      JSON.stringify(['system', 'context-access', accessType]),
      project,
      JSON.stringify({ accessType, timestamp: new Date().toISOString() })
    );
  } catch {
    // 실패해도 무시
  }
...
function getContinuityStats(project?: string): CallToolResult {
  try {
    ...
    `);
    const solutionCount = (project ? solutionStmt.get(project) : solutionStmt.get()) as { count: number };
    ...
    if (!project) {
      ...
      const projectCount = projectCountStmt.get() as { count: number };
      stats.totalProjectsWithContext = projectCount.count;
    }

    // 6. 평가 메트릭
    stats.evaluation = {
      contextSystemUsed: (stats.contextAccessLast7Days as unknown[])?.length > 0,
      hasTasks: (taskStats as unknown[]).length > 0,
...
function generateRecommendation(stats: Record<string, unknown>): string {
  const issues: string[] = [];
  ...
  if ((stats.contextAccessLast7Days as unknown[])?.length === 0) {
...
function getSafeOutputGuidelines(context?: string): CallToolResult {
  try {
    // 캐시된 패턴에서 가이드라인 생성
    const guidelines: string[] = [];
    const relevantPatterns: typeof contentFilterPatterns = [];
    ...
    if (context) {
      ...
      for (const pattern of contentFilterPatterns) {
        ...
        // 패턴 설명 매칭
        if (pattern.patternDescription.toLowerCase().split(' ').some(word =>
    ...
    // 파일 확장자별 특별 주의사항
    const extensionWarnings: Record<string, string> = {};
    for (const pattern of contentFilterPatterns) {
...
interface AutoLearnDecisionArgs {
  project: string;
  decision: string;
  reason: string;
  context?: string;
...
async function autoLearnDecision(args: AutoLearnDecisionArgs): Promise<CallToolResult> {
  try {
...
async function autoLearnFix(args: AutoLearnFixArgs): Promise<CallToolResult> {
  try {
...
interface AutoLearnPatternArgs {
  project: string;
  patternName: string;
  description: string;
  example?: string;
...
async function autoLearnPattern(args: AutoLearnPatternArgs): Promise<CallToolResult> {
  try {
...
interface AutoLearnDependencyArgs {
  project: string;
  dependency: string;
...
async function autoLearnDependency(args: AutoLearnDependencyArgs): Promise<CallToolResult> {
  try {
...
async function getSimilarIssues(
  errorOrIssue: string,
  project?: string,
  limit: number = 5
): Promise<CallToolResult> {
  try {
    ...
    if (embeddingPipeline) {
      ...
      const resultText = JSON.parse((result.content[0] as { text: string }).text);
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              solutions: resultText.results.map((r: Record<string, unknown>) => ({
                id: r.id,
  ...
  switch (name) {
    ...
    case 'get_session':
      return getSession(
        ...
        (args?.includeRaw as boolean) || false,
        (args?.maxContentLength as number) ?? 2000
      );

    case 'update_session':
      return updateSession(
        args?.project as string,
        args?.lastWork as string,
        args?.currentStatus as string | undefined,
        args?.nextTasks as string[] | undefined,
        args?.modifiedFiles as string[] | undefined,
        args?.issues as string[] | undefined,
        args?.verificationResult as string | undefined
      );

    case 'get_tech_stack':
      return getTechStack(args?.project as string);

    case 'run_verification':
      return runVerification(
        args?.project as string,
        args?.gates as string[] | undefined
      );

    case 'detect_platform':
      return detectPlatform(args?.project as string);
    ...
    case 'search_similar_work':
      return searchSimilarWork(
        args?.keyword as string,
        args?.project as string | undefined
      );

    case 'get_project_stats':
      return getProjectStats(args?.project as string | undefined);

    case 'record_work_pattern':
      return recordWorkPattern(
        args?.project as string,
        args?.workType as string,
        args?.description as string,
        args?.filesPattern as string | undefined,
        args?.success as boolean | undefined,
        args?.durationMinutes as number | undefined
      );

    case 'get_work_patterns':
      return getWorkPatterns(
        args?.project as string | undefined,
        args?.workType as string | undefined
      );
    ...
    case 'recall_memory':
      return recallMemory(
        ...
        args?.project as string | undefined,
        (args?.limit as number) || 10,
        (args?.minImportance as number) || 1,
        (args?.maxContentLength as number) ?? 500
      );

    case 'recall_by_timeframe':
      return recallByTimeframe(
    ...
    case 'search_by_tag':
      return searchByTag(
        args?.tags as string[],
        (args?.matchAll as boolean) || false,
        (args?.limit as number) || 20
      );
    ...
    case 'create_relation':
      return createRelation(
        ...
        (args?.strength as number) || 1.0
      );

    case 'find_connected_memories':
      return findConnectedMemories(
        args?.memoryId as number,
    ...
    case 'delete_memory':
      return deleteMemory(args?.memoryId as number);
    ...
    case 'semantic_search':
      return semanticSearch(
        args?.query as string,
        (args?.limit as number) || 10,
        (args?.minSimilarity as number) || 0.3,
        args?.memoryType as string | undefined,
        args?.project as string | undefined
      );

    case 'rebuild_embeddings':
      return rebuildEmbeddings((args?.force as boolean) || false);
    ...
    case 'collect_work_feedback':
      return collectWorkFeedback(
        args?.project as string,
        args?.workSummary as string,
        args?.feedbackType as string,
        args?.verificationPassed as boolean,
        args?.feedbackContent as string | undefined,
        args?.affectedTool as string | undefined,
        args?.duration as number | undefined
      );

    case 'get_pending_feedbacks':
      return getPendingFeedbacks(
        args?.feedbackType as string | undefined,
        (args?.limit as number) || 20
      );
    ...
    case 'record_filter_pattern':
      return recordFilterPattern(
        args?.patternType as string,
        args?.patternDescription as string,
        args?.fileExtension as string | undefined,
        args?.exampleContext as string | undefined,
        args?.mitigationStrategy as string | undefined
      );

    case 'get_filter_patterns':
      return getFilterPatterns(
        args?.patternType as string | undefined,
        args?.fileExtension as string | undefined
      );

    case 'get_safe_output_guidelines':
      return getSafeOutputGuidelines(args?.context as string | undefined);

    // ===== 자동 학습 시스템 =====
    case 'auto_learn_decision':
      return await autoLearnDecision(args as unknown as AutoLearnDecisionArgs);

    case 'auto_learn_fix':
      return await autoLearnFix(args as unknown as AutoLearnFixArgs);

    case 'auto_learn_pattern':
      return await autoLearnPattern(args as unknown as AutoLearnPatternArgs);

    case 'auto_learn_dependency':
      return await autoLearnDependency(args as unknown as AutoLearnDependencyArgs);

    case 'get_project_knowledge':
      return getProjectKnowledge(
        args?.project as string,
        args?.knowledgeType as string | undefined,
        args?.limit as number | undefined
      );

    case 'get_similar_issues':
      return await getSimilarIssues(
        args?.errorOrIssue as string,
        args?.project as string | undefined,
        args?.limit as number | undefined
      );
    ...
    case 'get_project_context':
      return getProjectContext(args?.project as string);

    case 'update_active_context':
      return updateActiveContext(
        args?.project as string,
        args?.currentState as string,
        args?.recentFiles as string[] | undefined,
        args?.blockers as string | undefined,
        args?.lastVerification as string | undefined
      );

    case 'init_project_context':
      return initProjectContext(
        args?.project as string,
        args?.techStack as Record<string, unknown> | undefined,
        args?.architectureDecisions as string[] | undefined,
        args?.codePatterns as string[] | undefined,
        args?.specialNotes as string | undefined
      );

    case 'update_architecture_decision':
      return updateArchitectureDecision(
        args?.project as string,
    ...
    case 'add_task':
      return addTask(
        ...
        args?.relatedFiles as string[] | undefined,
        args?.acceptanceCriteria as string | undefined
      );

    case 'complete_task':
      return completeTask(args?.taskId as number);
    ...
    case 'get_pending_tasks':
      return getPendingTasks(
        args?.project as string,
        args?.includeBlocked as boolean | undefined
      );

    // 에러 솔루션 아카이브
    case 'record_solution':
      return recordSolution(
        args?.errorSignature as string,
    ...
    case 'get_continuity_stats':
      return getContinuityStats(args?.project as string | undefined);
