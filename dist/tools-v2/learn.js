...
export async function handleLearn(args) {
    ...
    `);
        ...
        const result = stmt.run(data.content, memoryTypeMap[data.type], JSON.stringify(tags), data.project, data.type === 'decision' ? 8 : data.type === 'fix' ? 7 : 6, JSON.stringify(metadata));
...
function saveResolvedIssue(data) {
    ...
    `);
        const keywords = data.content
            ...
            .slice(0, 10)
            .join(' ');
        stmt.run(data.project, signature, data.content.substring(0, 1000), data.solution || '', JSON.stringify(data.files || []), keywords);
    }
    catch (e) {
        logger.error('Failed to save resolved issue', { error: String(e) }, 'learn');
    }
}
export async function handleRecallSolution(args) {
    ...
    `;
        const params = [`%${query.substring(0, 50)}%`, `%${query.split(' ')[0]}%`];
        if (project) {
        ...
        const issues = issuesStmt.all(...params);
        // 2. error 타입 메모리에서 검색
        let memoriesSql = `
    ...
    `;
        ...
        const memories = memoriesStmt.all(ftsQuery || query);
        const solutions = [
            ...issues.map(i => ({
