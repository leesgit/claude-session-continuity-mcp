...
// ===== 핸들러 =====
export function saveSessionHistory(project, lastWork, currentStatus, nextTasks, modifiedFiles, issues, verificationResult, durationMinutes) {
    try {
...
export function getSessionHistory(project, limit = 10, keyword) {
    try {
...
export function searchSimilarWork(keyword, project) {
    try {
        let query = `
...
export function recordWorkPattern(project, workType, description, filesPattern, success, durationMinutes) {
    try {
        // 기존 패턴이 있는지 확인
    ...
    `);
      ...
      `);
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `);
            const result = insertStmt.run(project, workType, description, filesPattern || null, success !== undefined ? (success ? 1 : 0) : 0, durationMinutes || 0);
            return {
                content: [{
                        type: 'text',
...
export function getWorkPatterns(project, workType) {
    try {
