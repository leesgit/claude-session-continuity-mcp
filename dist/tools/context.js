...
export function updateActiveContext(project, currentState, recentFiles, blockers, lastVerification) {
    try {
        const stmt = db.prepare(`
      INSERT OR REPLACE INTO active_context (project, current_state, recent_files, blockers, last_verification, updated_at)
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `);
        stmt.run(project, currentState, recentFiles ? JSON.stringify(recentFiles.slice(0, 10)) : null, blockers || null, lastVerification || null);
        return {
            content: [{
                    type: 'text',
...
export async function initProjectContext(project, techStack, architectureDecisions, codePatterns, specialNotes) {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `);
        stmt.run(project, finalTechStack ? JSON.stringify(finalTechStack) : null, architectureDecisions ? JSON.stringify(architectureDecisions.slice(0, 5)) : null, codePatterns ? JSON.stringify(codePatterns.slice(0, 5)) : null, specialNotes || null);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
export function updateArchitectureDecision(project, decision) {
    try {
        // 현재 결정 목록 가져오기
