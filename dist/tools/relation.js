...
// ===== 핸들러 =====
export function createRelation(sourceId, targetId, relationType, strength = 1.0) {
    try {
...
export function findConnectedMemories(memoryId, depth = 1, relationType) {
    try {
      ...
      `;
            ...
            const outgoingRows = outgoingStmt.all(...outgoingParams);
      ...
      `;
            ...
            const incomingRows = incomingStmt.all(...incomingParams);
        ...
        explore(memoryId, 1);
        // 중복 제거
        const uniqueConnections = connections.filter((conn, index, self) => index === self.findIndex(c => c.memoryId === conn.memoryId && c.relationType === conn.relationType));
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
