...
export async function getSession(project, includeRaw = false, maxContentLength = 2000) {
...
export async function updateSession(project, lastWork, currentStatus, nextTasks, modifiedFiles, issues, verificationResult) {
    const sessionPath = path.join(APPS_DIR, project, 'docs', 'SESSION.md');
...
export async function getTechStack(project) {
...
export async function runVerification(project, gates) {
...
export async function detectPlatform(project) {
    const projectPath = path.join(APPS_DIR, project);
    const checks = {
        'package.json': 'Web',
