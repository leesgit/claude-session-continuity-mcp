...
**Solution**: ${s.solution}
        ...
        `).run(content, memoryType, tags ? JSON.stringify(tags) : null, project || null, importance, null);
          ...
          `).run(memoryId, relatedTo);
            ...
            case 'memory_search': {
              ...
              LIMIT 100
            `).all(minImportance, ...(memoryType && memoryType !== 'all' ? [memoryType] : []), ...(project ? [project] : []));
                        const scored = allMemories.map(m => {
                            if (!m.embedding)
                                return { ...m, similarity: 0 };
                            const emb = Array.from(new Float32Array(m.embedding.buffer));
                            return { ...m, similarity: cosineSimilarity(queryEmb, emb) };
                        });
                        scored.sort((a, b) => b.similarity - a.similarity);
                        results = scored.filter(m => m.similarity > 0.3).slice(0, limit);
                    }
                }
                else {
                    // FTS5 키워드 검색
          ...
          `).run();
            ...
            case 'memory_related': {
                ...
                const related = [];
                // 기준 메모리 조회
                const baseMemory = db.prepare('SELECT * FROM memories WHERE id = ?').get(memoryId);
                if (!baseMemory) {
          ...
          `).get(memoryId);
            ...
            `).all(memoryId);
                        const scored = allMemories.map(m => ({
                            ...
                            similarity: cosineSimilarity(baseVec, Array.from(new Float32Array(m.embedding.buffer)))
                        }));
                        scored.sort((a, b) => b.similarity - a.similarity);
                ...
                return {
                    content: [{
                            ...
                            text: JSON.stringify({
                                baseMemory: {
                                    id: baseMemory.id,
                                    content: baseMemory.content.substring(0, 200),
                    ...
                    content: [{
                            ...
                            text: JSON.stringify({
                                summary: {
                                    totalMemories,
                                    totalRelations,
                                    embeddingsCount: db.prepare('SELECT COUNT(*) as count FROM embeddings_v4 WHERE entity_type = ?').get('memory').count
                                },
        ...
        `).run(sourceId, targetId, relation, strength);
            ...
            case 'graph_explore': {
                const memoryId = args.memoryId;
                const maxDepth = Math.min(args.depth || 2, 4);
                const relationFilter = args.relation;
                const direction = args.direction || 'both';
                const visited = new Set();
                const nodes = [];
                const edges = [];
                function explore(currentId, currentDepth) {
                    if (currentDepth > maxDepth || visited.has(currentId))
                        return;
                    visited.add(currentId);
                    const memory = db.prepare('SELECT id, content, memory_type FROM memories WHERE id = ?').get(currentId);
                    if (memory) {
                        nodes.push({
                            id: memory.id,
                    ...
                    if (direction === 'outgoing' || direction === 'both') {
                        ...
                        for (const r of outgoing) {
                            ...
                            explore(r.target_id, currentDepth + 1);
                        }
                    }
                    // 들어오는 관계
                    if (direction === 'incoming' || direction === 'both') {
                        ...
                        const incoming = db.prepare(sql).all(...params);
                        for (const r of incoming) {
                            ...
                            explore(r.source_id, currentDepth + 1);
                        }
                    }
                }
                explore(memoryId, 0);
                // 중복 엣지 제거
                const uniqueEdges = edges.filter((edge, index, self) => index === self.findIndex(e => e.from === edge.from && e.to === edge.to && e.relation === edge.relation));
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
...
async function generateProjectContext(project) {
    ...
    if (activeRow) {
        lines.push(`## 현재 상태`);
        if (activeRow.current_state)
            lines.push(`**상태**: ${activeRow.current_state}`);
        if (activeRow.blockers)
            lines.push(`**🚧 블로커**: ${activeRow.blockers}`);
        if (activeRow.last_verification)
            lines.push(`**마지막 검증**: ${activeRow.last_verification}`);
        if (activeRow.recent_files) {
            const files = JSON.parse(activeRow.recent_files);
            if (files.length > 0)
                lines.push(`**최근 수정 파일**: ${files.join(', ')}`);
        }
        lines.push('');
    }
    // 3. 최근 세션
  ...
  `).get(project);
    if (lastSession) {
        ...
        if (lastSession.current_status)
            lines.push(`**진행**: ${lastSession.current_status}`);
        if (lastSession.next_tasks) {
            const tasks = JSON.parse(lastSession.next_tasks);
            if (tasks.length > 0)
                lines.push(`**다음 할 일**: ${tasks.join(', ')}`);
        }
        lines.push('');
    }
    // 4. 미완료 태스크 (상위 5개)
...
async function generateRecentMemories(project, limit = 10) {
    ...
    for (const mem of memories) {
        lines.push(`### ${typeIcons[mem.memory_type] || '💭'} (${mem.project || 'global'})`);
        lines.push(`> ${mem.content}`);
        lines.push(`_중요도: ${mem.importance} | ${mem.created_at}_\n`);
    }
    return lines.join('\n');
}
async function generateWorkContext(project) {
    const lines = [];
    // 프로젝트 컨텍스트
